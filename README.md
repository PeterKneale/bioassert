...
| `suggest <file>`  | Inspect a file and write a starting set of suggested assertions to `<file>.assertions.txt` for you to review. |

`run` is the primary workflow: keep your checks in a version-controlled assertions file and run them as a pipeline step.
`assert` is handy for one-off checks and quick experimentation at the shell. `suggest` bootstraps an assertions file
for a new output, so you start from sensible defaults instead of a blank page (see
[Suggesting a starting set](#suggesting-a-starting-set)).
...
Lines beginning with `#` are comments and blank lines are ignored. A `#` later on a line starts an inline comment
(everything after it is ignored), which is how `suggest` annotates its output; quote a value that must contain a
literal `#`. Whitespace between fields is flexible, so columns can be aligned for readability. Pass the file to
`bioassert run`.
...
### Suggesting a starting set

Writing an assertions file from scratch for a new output is tedious when you do not yet know its column count, record
count, or size. `bioassert suggest <file>` inspects the file and writes a starting set of assertions to
`<file>.assertions.txt` (or `--output FILE`), which you then review and tighten:

```bash
bioassert suggest junctions.tsv          # writes junctions.tsv.assertions.txt
bioassert suggest junctions.tsv -o checks.txt
bioassert suggest junctions.tsv --force  # overwrite an existing output
```

The output is ordinary assertion syntax with inline `#` comments, so `bioassert run` reads it back directly:
...
The suggestions are **guesses from the file's current state, not verified expectations**: every file gets
`file.exists` and a non-empty `file.size` check; schema-like counts (delimited columns, FASTA records, BAM `@SQ`) are
pinned with `eq`; and quantities that vary run to run (delimited rows, FASTA total length) get a `gte`/`lte` band of
+/- 50%. The defaults chosen per file type are driven by its extension. `suggest` will not overwrite an existing
output unless you pass `--force`, since that file is meant to be hand-edited.
