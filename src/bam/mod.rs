...
//! read-group or program (`@PG`) tag value, a presence flag, or an `@HD` field. The
//! `bam.header.*` namespace leaves room for future record-level metrics (e.g. `bam.records.*`).
...
pub use program::{BamProgramPresentExecutor, BamProgramTagExecutor};
