...
fn evaluate(assertion: &Assertion) -> Result<Evaluation, BioAssertError> {
    if let Some(guard) = &assertion.guard {
        ...
        if !active {
            let reason = if guard.negate { "condition met" } else { "condition not met" };
...
fn dispatch(metric: &str, request: &AssertionRequest) -> Result<(bool, Value), BioAssertError> {
    if let Some(e) = FileExistsExecutor::try_parse(metric) { return run(e, request); }
    ...
    if let Some(e) = DelimitedColumnAllExecutor::try_parse(metric) { return run(e, request); }
    if let Some(e) = BamCountExecutor::try_parse(metric) { return run(e, request); }
    if let Some(e) = BamHeaderFieldExecutor::try_parse(metric) { return run(e, request); }
    if let Some(e) = BamReadGroupPresentExecutor::try_parse(metric) { return run(e, request); }
    if let Some(e) = BamReadGroupTagExecutor::try_parse(metric) { return run(e, request); }
...
fn run<E: AssertionExecutor>(executor: E, request: &AssertionRequest) -> Result<(bool, Value), BioAssertError> {
...
mod tests {
    ...
    fn assertion(file: &str, metric: &str, comparator: &str, expected: &str, guard: Option<Guard>) -> Assertion {
        Assertion {
            file: file.to_string(),
    ...
    fn guard_that_errors_is_reported_as_error() {
        ...
        assert!(result.message.contains("guard could not be evaluated"), "message was: {}", result.message);
    }
    ...
    fn assertion_without_a_guard_runs_normally() {
        let result = execute(assertion("tests/data/example.tsv", "tsv.columns.count", "eq", "3", None));
        assert_eq!(result.outcome, Outcome::Pass);
    }
}
